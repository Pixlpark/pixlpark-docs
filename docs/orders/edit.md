...
## Описание
* После оформления заказа клиенту и сотруникам компании будут отправлены соответствующие уведомления. В уведомлении для сотрудника помимо информации о заказе будут размещены ссылки в карточку заказа и на архив с файлами для печати. Однако, это произойдет не сразу, а в промежутке от 5 до 20 минут. Это время потребуется для подготовки файлов заказа и упаковки их в архив.
...
* Он состоит из пяти основных блоков:
    ...
    + __Лента обсуждения__ - cписок сообщений от клиента и сотрудников компании, упорядоченных по времени; сотрудник компании может запросить утверждение клиентом своего сообщения (например, если он содержит предложенный вариант дизайна макета); клиент может либо утвердить сообщение сотрудника, либо отклонить его.
