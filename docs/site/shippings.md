...
* Также на странице можно:
    + Отфильтровать доставки по их доступности и типу.
...
### Связь с опциями
* В данном разделе отображается список опций, их позиции и продукты, с которыми они связаны. Список опций можно фильтровать по группам печати.
* Каждую позицию опции можно сделать доступной, либо недоступной для данного способа доставки.
...
#### Список зон доставки
* В данном разделе отображается список зон, куда может быть осуществлена доставка. 
* Под зонами подразумеваются населенные пункты выбранного на странице доставки города. Соответственно, после выбора города будет происходить поиск зон курьерской доставки с таким же значением в поле "__Город__".
![](../_media/site/shippings-courier-addresses.png)

* Также на странице можно:
    + Добавить новую зону.
    + Перейти в карточку зоны.
    + Включить или выключить любую зону.
    + Задать стоимость доставки для выделенных зон.
    + Удалить выделенные зоны.

#### Карточка зоны доставки
* Карточка зоны доставки содержит два блока:
    + __Основная информация__ - доступность зоны доставки на сайте и в приложении.
    ![](../_media/site/shippings-courier-address-general-info.png)
    + __Полный адрес__ - адрес зоны доставки, где определяющим значением является поле "__Город__", по которому идет выбор подходящей зоны на странице доставки.
    ![](../_media/site/shippings-courier-address-general.png)
    + __Стоимость доставки__ - алгоритм расчета стоимости доставки в зависимости от веса заказа. 
    ![](../_media/site/shippings-courier-address-price.png)
...
> Подробнее о настройке интеграции с TK можно узнать в __[разделе](/integration/shippings?id=СДЭК)__. 
> Подробнее о настройке без интеграции с ТК можно узнать в __[разделе](/integration/shippings?id=ТК-без-интеграции)__.

### Стоимость доставки
* В данном разделе задается алгоритм расчета стоимости доставки.
![](../_media/site/shippings-postal-price.png ':size=70%')
...
* В карточке пункта выдачи есть три блока:
    ...
    ![](../_media/site/shippings-postal-point-price.png)
...
> Для корректной работы доставки необходимо заполнить хотя бы один ключ для карты.
>    + Получить API ключ Яндекс.Карты можно в разделе "__JavaScript API и HTTP Геокодер__" [кабинета разработчика](https://developer.tech.yandex.ru/) нажав "__Получить ключ__".
>    + Получить API ключ Google.Maps можно в [Google Maps Platform](https://mapsplatform.google.com). Для корректной работы должны быть включены "__Geocoding API__", "__Map JavaScript API__", "__Places API__".
