## Страницы
* Страница - это страница сайта, доступная по уникальному адресу. Для одной страницы может быть создано несколько локализаций.

* __Содержание__
    + [Список страниц](/cms/pages?id=Список-страниц)
    + [Основная информация страницы](/cms/pages?id=Основная-информация-страницы)
...
* Ниже перечислены параметры страницы.
...
### Редактор страницы
* Страница формируется из элементов - секций. По умолчанию содержимое задается в виде HTML-разметки, которая также может содержать стили и скрипты. Добавить новый контент или изменить существующий можно во вкладке "Редактор".
...
* Блок - это произвольная HTML-разметка (+ стили + скрипты), которая может быть многократно использована на различных страницах сайта. Использование блоков позволяет минимизировать разметку страниц, делая ее более читабельной и лаконичной. Блок можно локализовать.

* __Содержание__
    + [Список блоков](/cms/pages?id=Список-блоков) 
    + [Основная информация блока](/cms/pages?id=Основная-информация-блока)
    + [Редактор блока](/cms/pages?id=Редактор-блока) 
...
* __Вывод на странице__ - константа, позволяющая выводить на страницу содержимое блока.Название константы нужно указать в коде страницы, чтобы блок отобразился.
* Через "зависимости" можно посмотреть список страниц, в которых используется данный блок.


* К блоку могут быть применены следующие настройки:
* __Блок доступен на сайте__ - настройка отображения блока на сайте.
* __Не дублировать при копировании сайта__ - выключиьт копирование блока при дкблировании сайта.
* __Ограничить блок следующими группами клиентов__ - позволяет выбрать группы клиентов, которые будут видеть блок. Подробнее о группах можно прочесть в разделе ["Группы"](../customers/groups.md).

### Редактор блока
* Блоки создаются аналогично страницам. Во вкладке "Редактор" задается HTML-разметка блока, также могут быть использованы стили и скрипты.
![](../_media/cms/block-editor.png)



## Слои

* Слой - это наборы HTML-разметки, которые автоматически добавляются к предзаданным секциям страницы в момент их отрисовки при соответствии адреса страницы заданной в маске слоя. Ниже перечислены основные параметры блока.

* __Содержание__
    + [Список слоев](/cms/pages?id=Список-слоев)
    + [Основная информация слоя](/cms/pages?id=Основная-информация-слоя)
    + [Работа с маской](/cms/pages?id=Работа-с-маской)
    + [Редактор слоя](/cms/pages?id=Редактор-слоя)
...
* Также в этом разделе можно:
    ...
    + Включить или выключить доступность слоя на сайте.
    + Удалить выделенные слои.
* ![](../_media/cms/layers.png)

### Основная информация слоя
