# Внедрение редактора во внешний магазин
* Любой редактор сервиса Pixlpark (включая механизм выбора шаблонов или принтов) может быть встроен во внешний сайт (интернет-магазин) с небольшой доработкой последнего.<br>
...
## Внедрение редактора дизайнов во внешний сайт

* Редактор дизайнов возможно использовать на внешнем (относительно сайта в сервисе Pixlpark) сайте.
* Для настройки интеграции необходимо выполнить следующие действия.

### Требования к настройке сайта в сервисе Pixlpark
* Сайт в сервисе Pixlpark должен быть подготовлен согласно следующим требованиям:
    + Сайт должен [работать](https://docs.pixlpark.ru/#/site/domains) на протоколе HTTPS.
    + Сайт находится на поддомене внешнего сайта, куда необходимо интегрировать редактор.
    + В подразделе "__Основные настройки__" раздела "__Настройки / Доступ__" включена опция "__Разрешить встраивать сайт через IFRAME на внешние ресурсы__" и указан адрес внешнего сайта с протоколом.
    + В подразделе "__Доступность по API__" раздела "__Настройки / Доступ__" включена опция "__Разрешить доступ к данным через API__".
    + Сайт содержит доступный настроенный продукт с уникальной ссылкой в редактор, который необходимо интегрировать.
...
* Интеграция необходимого редактора с внешним сайтом осуществляется при помощи идентификатора продукта, который может быть двух видов:
    + "__ID__" - уникальный идентификатор продукта.
        > При удалении с сайта в сервисе Pixlpark продукта с указанным в интегрируемом редакторе ID на внешнем сайте редактор не будет работать.
    + "__categoryUrlName__" и "__productUrlName__" - идентификатор категории продукта и идентификатор продукта в URL.
        > При отсутствии указания "__productUrlName__" редактор будет инициализирован первым доступным продуктом категории продукта.

### Требования к настройке внешнего сайта
* Для настройки интеграции необходимо, чтобы внешний сайт работал на протоколе HTTPS.

### Реализация аутентификации
* Внешний редактор запускается в двух режимах:
    + __Гость__.
    ...
    + __Пользователь__.
        > В данном режиме доступны все функции редактора. Режим запускается при условии установления параметра `config.auth.userToken` отличного от пустой строки или от `null` в конфигурации редактора. Далее, указанный токен будет проверен на сервере. В случае, если валидация токена не осуществится, редактор прикратит свою работу.

### Получение токена
* Для получения токена пользователя необходимо реализовать взаимодейстие с API Pixlpark.
* В случае, если пользователь аутентифицирован на внешнем ресурсе , необходимо получить / создать (при отсутствии данного пользователя на сайте в сервисе Pixlpark) через API Pixlpark и получить внешний токен.
    + `GET api.pixlpark.com/users/byEmail` и/или `POST api.pixlpark.com/users/create`.
    + `GET /users/{id}/frontendToken`.
* В случае, если пользователь не аутентифицирован на внешнем ресурсе, необходимо его зарегистрировать и создать его копию на сайте в сервисе Pixlpark.
    + `GET api.pixlpark.com/users/byEmail` и/или `POST api.pixlpark.com/users/create`.
    + `GET /users/{id}/frontendToken`.

### Встраивание редактора на страницу внешнего сайта
* Для встраивания редактора на страницу внешнего сайта необходимо создать контейнер согласно коду `<div id="editorContainer" class="editor-container loading-wheel"></div>` и добавить его на необходимую страницу внешнего сайта.
...
```
* После, задать конфиругацию редактора на странице внешнего сайта при помощи кода:
> Скрипт необходимо расположить после скрипта загрузки редактора.
```
    const container = document.getElementById('editorContainer');
    const designEditorConfig = {
        product: {
            productUrlName: "идентификатор_продукта",
            categoryUrlName: "идентификатор_категории_продукта",
        },
        auth: {
            getToken: () => {
...
```
> Результатом работы редактора является добавленный в корзину продукт. Также необходимо реализовать метод `onCartItemCreated` для определения дальнейших действий после добавления продукта в корзину.
...
```
    interface IDesignEditorConfig {   
        ...
        ui?: {
            ...
            // Настройка верхней панели редактора. Если установить значение null - панель не выводится.
            header?: {
                // Отображать кнопку домой
                // по умолчанию: true
                homeButton?: boolean;

                // Изображение для кнопки домой. Если значение не указано или null - будет установлен логотип из настройки сайта
                // по умолчанию: null
        ...
        events?: {
            ...
            // Вызывается после того как редактор загрузился и готов к работе
            onReady?: () => void,
        ...
        // Настройка товара
        product: {
            // ID продукта. Можно указать или ID, или categoryUrlName и productUrlName
            id?: number,
        ...
        // Аутентификация
        auth: {
            // Этот метод вызывается если не задан userToken и необходим зарегистрированный пользователь
            getToken: () => Promise<string>,

            // Токен зарегистрированного пользователя. При его указании редактор попробует войти под этим токеном,
