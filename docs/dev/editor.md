...
* Редактор встраивается в четыре этапа:
...
* Результатом работы редактора является переданный в корзину продукт. При этом необходимо реализовать хэндлер с целью определения дальнейших действий.
* Затем, на странице корзины необходимо создать заказ реализовав метод __Create Order__ согласно [API Pixlpark](https://docs.pixlpark.ru/#/dev/api) с указанием значения __shippingId__.
> Значение __shippingId__ может быть равно нулю, либо уникальному номеру созданного специально для данного продукта способа доставки. Номер способа доставки можно узнать в адресной строке карточки доставки в разделе "__Настройка / Доставка / Способы доставки__" в панели управления Pixlpark.
* ![](../_media/dev/editor-shipping-id.png)
* После создания заказа осуществляется рендеринг его файлов на стороне Pixlpark. Его завершение можно контролировать самостоятельно либо настроить уведомление об изменении статуса рендеринга при помощи [вебхуков](/dev/api#Работа-с-Вебхуками). Для этого в подразделе "__Настройка вебхуков__" раздела "__Маргетинг / Уведомления__" в панели управления Pixlpark необходимо добавить вебхук для события "__Заказ: статус рендеринга изменен__" и указать URL-адрес для отправления запроса.
![](../_media/dev/editor-webhook.png)
* Затем, нажать "__Сохранить__". 
* После создания вебхука нажать "__Сохранить__" и сбросить кэш сайта.