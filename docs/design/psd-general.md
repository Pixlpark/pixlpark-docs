...
* В макете можно использовать только 32-битное цветовое пространство и любую цветовую модель, однако файл для печати будет сформирован в модели RGB.
...
* В наименовании слоев можно использовать следующие ключи:
    ...
    + `[template-preview]` – не отображает слой в редакторе, однако использует его в качестве обложки шаблона при загрузке макета в Pixlpark;
...
1. Создать растровый слой с двумя ключами `[collage:n]` (**n** - количество фотографий в коллаже) и `[path:pathName]`.
...
1. В панели управления Pixlpark в разделе ["Печать / Дизайн / Календари"](/print/designs?id=Календари) создать собственные дизайны календарных сеток или оставить для использования стандартные.
...
* Вы можете скачать [пример макета](https://admin.pixlpark.com/content/pxp-template-download/18103595?secret=4e95f489f54f2fe625b8c8f9e34aee35) или [посмотреть его в редакторе](https://demo.pixlpark.ru/printing/wall-calendars-smart/a4-vert/template-6324454/editor?q=1&oq=0&p=13&ws=d751713988987e9331980363e24189ce).
