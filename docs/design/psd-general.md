...
1. В макете использовать следующие ключи:
    + `[template-preview]` – группа слоев, в которой находятся превью обложек календаря. Группа нужна для того, чтобы в превью год календаря на странице выбора шаблона менялся автоматически. Наименование слоев в группе должно иметь вид **[2023-0]**, где "2023" – год, "0" – обложка.
...
* __Содержание__
    + [Создание контура](/design/psd-general?id=Создание-контура)
